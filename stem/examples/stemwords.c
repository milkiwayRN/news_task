...
#define INC 10
    ...
    while(1) {
	    ...
	    int inlen = 0;
            while(1) {
                if (ch == '\n' || ch == EOF || ch == '\t' || ch == ' '|| ch==',' || ch=='.' || ch=='\"' || ch=='-' || ch=='-' || ch=='(' 
					|| ch == ')'  ) break;
                if (i == lim) {
                    sb_symbol * newb;
		    newb = (sb_symbol *)
                ...
                else
		{
			...
			/*
			if (isstopword(b, i, sl) == 0)
			*/
			fputs((char *)stemmed, f_out);
			if (ch != EOF && ch != '\"' && ch != ':' && ch != ',' && ch != '.' && ch != '-' && ch != '-' && ch != '(' && ch != ')' )
				putc(ch, f_out);
			else
				putc(32, f_out);
...
{/*
	struct stop_list* slist = (struct stop_list*)malloc(sizeof(struct stop_list));

	create_stop_list(slist);

	read_stop_list(slist);
*/
    char * in = 0;
    char * out = 0;
    FILE * f_in;
